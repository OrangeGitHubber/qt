...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  market_fractional: {
    ...
    explain:
      "Two things at once, and which of them matters depends on the asset. FOR STOCKS: off by default, the strategy sends price-protected marketable limit orders and buys whole shares, so a small $ per trade can't buy an expensive name at all ($100 buys 0 whole shares of a $400 stock and the buy is skipped). Turn it on and it sends plain market orders sized by dollar amount, so that $100 buys about a quarter of a share and fills immediately. FOR CRYPTO: the fractional half does nothing — crypto is always bought in fractions either way — so all this changes is the order type. The trade-off is the same for both: a market order takes whatever price is there, with no limit protecting you on a fast or thin move, while a limit order protects the price but can sit unfilled. Best on liquid names you'd otherwise miss; leave it off for thin small-caps where the spread can bite.",
